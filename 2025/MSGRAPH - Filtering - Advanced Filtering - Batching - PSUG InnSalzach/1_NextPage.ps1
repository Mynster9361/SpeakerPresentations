...
foreach ($user in $users.value) {
    ...
    $uri = "https://graph.microsoft.com/v1.0/users/$($user.id)/appRoleAssignments?`$top=1"
    $appRoleAssignments = Invoke-RestMethod -Method Get -Uri $uri -Headers $authHeaders
    $appRoleAssignments.value | ForEach-Object {
        Write-Host "App Role Assignment: $($_.id) - Principal ID: $($_.principalId) - Resource ID: $($_.resourceId)"
    }
