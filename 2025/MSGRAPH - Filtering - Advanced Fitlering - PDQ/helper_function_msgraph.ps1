...
function Invoke-GraphApiRequest {
    ...
    do {
        ...
        try {
            if ($Body) {
                # Ensure Content-Type header is set for POST/PUT/PATCH requests
                if ($Method -in @('POST', 'PUT', 'PATCH') -and -not $Headers.ContainsKey('Content-Type')) {
                    $Headers['Content-Type'] = 'application/json'
                }

                # If Body is already a string (JSON), use it as-is; otherwise convert to JSON
                if ($Body -is [string]) {
                    $requestBody = $Body
                }
                else {
                ...
                $response = Invoke-RestMethod -Method $Method -Uri $Uri -Headers $Headers -Body $requestBody
            }
            else {
